...
const BookingDetails = () => {
  const { id } = useParams();
  const { getAllBookingsByGuestId, cancelBooking } = useContext(RoomContext);
  const [filter, setFilter] = useState("upcoming"); // State to manage the active filter
  const { data, error, isLoading, refetch } = useQuery({
    queryKey: ["booking", id],
    queryFn: () => getAllBookingsByGuestId(id),
  });
  const [uplcommingLength, setUplcommingLength] = useState(0);
  ...
  useEffect(() => {
    if (data?.status) {
      ...
      setUplcommingLength(data?.bookings?.upcoming?.length);
      setCompletedLength(data?.bookings?.completed?.length);
      setCancelledLength(data?.bookings?.cancelled?.length);
    } else {
            ...
            bookings[filter].map((booking) => (
              ...
              >
                ...
                <div className="flex gap-2 mt-2">
                  <Link
                    to={`/admin/dashboard/users/bookings/details/${booking._id}`}
